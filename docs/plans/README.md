...
| 2026-09-17 | [Phase 4 -- reconciliation evidence correctness](2026-09-17-phase-4-reconciliation-evidence-correctness.md) | Built (S15, S1, S3, S4/M4, S2, M1/M2, M21, B4-followup; full suite + `cli validate` green; demo-evidence diff recorded in the plan's own Implementation note -- finding/tie counts and ids unchanged, evidence fields and recitation genuinely changed as expected; deploy still outstanding) |
| 2026-09-17 | [Phase 5 -- RPS hardening](2026-09-17-phase-5-rps-hardening.md) | Built and deployed (S9, S10, S11, M14, M15; M16's RPS half was already done in Phase 1, dropped from scope with a citation; full suite + `cli validate` green; demo-evidence diff confirmed empty, no `DEMO_FIXTURE_VERSION` bump needed) |
| 2026-09-17 | [Phase 6 -- ops, performance, hygiene](2026-09-17-phase-6-ops-performance-hygiene.md) | Built (S6-full, M11, M12, M8, M9, M22, M23, Cosmetics, S16-docs-only; `reopen_case`/`TieKind.DECLARED_AFFILIATION_DIRECT` already resolved by earlier phases, dropped from scope with a citation; M8's design corrected from a three-way to a two-way notice split during implementation -- see the plan's own Implementation note; full suite + `cli validate` green; demo-evidence diff confirmed empty; deploy still outstanding) |
