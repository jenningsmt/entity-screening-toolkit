...
- Produce confidence-scored, evidence-traceable output an auditor could follow back to source records — explainable, traceable to source evidence, and defensible to a compliance office, the bar any credible compliance-technology system needs to clear.
...
- Acceptance: a documented test set of "known-difficult" name pairs (true positives that look different, true negatives that look similar) exists and is checked on every run — standard sanctions-screening QA terminology ("known-difficult entities").
...
**Core language stays Python.** The instinct to move beyond the ED tools' stack is right, but the fix isn't the language — Python is the industry-standard tool for exactly this problem domain. OpenSanctions itself is built on Python (their own dedupe library, `nomenklatura`/Follow the Money); Splink, the well-regarded open-source Bayesian record-linkage library used by the UK Ministry of Justice and others, is Python. Moving the core engine to another language would trade away that ecosystem for no real benefit, and would read as a worse signal to the compliance-tech and due-diligence audience this project is aimed at, who'd expect Python fluency for this class of work, not a rewrite in Go or Java.
...
## 9a. Architecture & Process Adjustments (added while still in V1)

A few architecture and process changes were made while still in V1, specifically because V1 isn't far enough along for them to be costly to make now. Each is defended on its own engineering merits below — see the "deliberately not adopted" list at the end for changes that were considered and didn't clear that bar.

**Architecture change — add a FastAPI layer under Streamlit.** A REST API layer under Streamlit is standard practice for this class of application — due-diligence and data-visualization platforms in this space are commonly built on Python/FastAPI with a separate frontend, with "API design"/"REST API interactions"/"microservices" as common expectations for this kind of role. Streamlit remains the demo UI, but it should be a thin consumer of a REST API sitting over the `entity_screening` engine package, not the entire application. This is better architecture on its own merits, and it keeps the door open to a real frontend later without rework.

**Epic J: Evidence-Grounded Explanation Generation (V3-adjacent) — built, see below.** The project as scoped is pure structured matching (fuzzy names, confidence scores). A natural next step for this class of due-diligence tool is a RAG pipeline and an AI-agent layer on top of the entity knowledge base — generating a natural-language explanation of a flagged match, grounded strictly in its retrieved evidence.
...
- **Built 2026-09-15** (`docs/plans/2026-09-15-epic-j-evidence-grounded-explanation.md`), against `Finding`/`ConcernTie` (the HB127/COI worksheet — RPS's `ScreeningMatch` and the batch pipeline's `ScreeningHit` share the same evidence shape but are deferred as UI/route wiring only). The acceptance criterion above ("never allowed to assert anything beyond what that evidence supports") turned out to need two distinct guarantees, not one: a citation-grounding check (every claim in the one generated sentence resolves back into the evidence document, via Claude's native document-citation feature) *and* a separate forbidden-vocabulary check on the generated prose (a fact can be perfectly true and cited and still editorialize — "this appears serious" — which grounding alone doesn't catch). Most of an explanation is fully templated in Python from typed fields, not generated at all; exactly one short synthesis sentence is genuinely composed by the model, and a sentence that fails either check is never persisted, never persisted-with-a-warning-flag.
...
- **Formal RMF/ATO accreditation** — a real DoD/IC compliance process, entirely irrelevant without an actual government customer.
...
- **Naming a specific target employer or product in the public repo.** Good architecture decisions stand on their own engineering merits; tying them explicitly to a specific company's hiring signals reads as reverse-engineering an interview rather than genuine engineering interest. That kind of "why" belongs in a cover letter or an interview conversation, not baked into the public artifact.
...
- **The primary entry point changes** from a dataset to a subject. Section 6's epics assume a corpus; the case model does not exist. The batch path itself is not superseded — see "What survives" below.
...
- **The ownership graph (Epic C) finally earns its place.** A declared employer whose ultimate parent sits on a concern list is the **concern-tie** shape this use case most needs — a §51B.151(b) background-check observation, not a §51B.153 non-disclosure — and it is unreachable by name-matching the declared name alone. (The first slice mislabelled it as a `Finding`; `docs/plans/2026-09-06-concern-ties-as-a-distinct-observation.md` splits the two.)

- **The batch orchestration survives, repositioned.** `pipeline.run_screening` is not legacy awaiting deletion. The use-case document's case-reopening rule (§5) requires a periodic sweep of the closed-case population against updated reference data — a new concern-list release, a country entering the rolling three-ATA adversary window, newly indexed publications — to surface which closed cases now carry findings. That is the same orchestration pointed at a case population rather than an award corpus, and it is also a recognisable NSPM-33 periodic-review function. What retires inside it is narrower than "the corpus path": `resolve_entities_from_nsf`, which groups award records into organisational entities, has no role under the case model, while `NSFAwardIngester` repoints as a discovery source keyed on PI name (use-case document §7).

**The corpus user interface does not survive.** The case worksheet becomes the only visitor-facing view. The batch path stays reachable through the CLI and the API, unadvertised, so it remains exercised and CI-covered without presenting a second front door demonstrating the question this section exists to move away from. The remediation pass's protections carry over rather than being stranded — the action gate, file allowlist, rate limiting and noindex guard any mutating operation, and the self-healing `run_id="demo"` becomes a self-healing demo *case*.

This arrangement is resolved — the batch path formally renamed, re-documented and tested against a case population — when the case path can produce a closed, exported investigative file. Stated as a trigger rather than an intention, because code kept with a vague plan to revisit it tends not to get revisited.
