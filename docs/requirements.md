...
**New epic — Epic J: Evidence-Grounded Explanation Generation (V3-adjacent).** The project as scoped is pure structured matching (fuzzy names, confidence scores). A natural next step for this class of due-diligence tool is a RAG pipeline and an AI-agent layer on top of the entity knowledge base — generating a natural-language explanation of a flagged match, grounded strictly in its retrieved evidence.
